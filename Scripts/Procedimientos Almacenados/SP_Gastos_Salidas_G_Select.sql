...
BEGIN
	...
	select a.Id_Activo,a.Nombre_Interno,E.Nombre_Empresa,
		...
		sum(isnull(sd.Importe_D,0))+sum(isnull(trans.Importe_D,0))+sum(isnull(Caseta.Importe_D,0))+sum(isnull(PFP.Importe_D,0))+sum(isnull(tReten1.Importe_D,0))+sum(isnull(pens.Importe_D,0))+sum(isnull(lav.Importe_D,0))+sum(isnull(Ther.Importe_D,0))+sum(isnull(otro.Importe_D,0))+sum(isnull(sv.Importe_P,0))+sum(isnull(sh.Importe_P,0)) as Gasto_Total_D, 
		sum(isnull(sf.Importe_D,0))- (sum(isnull(sd.Importe_D,0))+sum(isnull(trans.Importe_D,0))+sum(isnull(Caseta.Importe_D,0))+sum(isnull(PFP.Importe_D,0))+sum(isnull(tReten1.Importe_D,0))+sum(isnull(pens.Importe_D,0))+sum(isnull(lav.Importe_D,0))+sum(isnull(Ther.Importe_P,0))+sum(isnull(otro.Importe_P,0))+sum(isnull(sv.Importe_P,0))+sum(isnull(sh.Importe_P,0))) as Utilidad_Total_D
