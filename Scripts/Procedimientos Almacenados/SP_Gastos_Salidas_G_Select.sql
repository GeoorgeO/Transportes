...
BEGIN
	...
	select a.Id_Activo,a.Nombre_Interno,E.Nombre_Empresa,
		...
		max(Gas.Importe_P)/(select count(Id_Activo) from transportes.dbo.activos where Status='ACTIVO' and Activo_Primario=1 and Id_Empresa=@Id_empresa) as GastoIndirecto_P,
		max(Gas.Importe_D)/(select count(Id_Activo) from transportes.dbo.activos where Status='ACTIVO' and Activo_Primario=1 and Id_Empresa=@Id_empresa) as GastoIndirecto_D
